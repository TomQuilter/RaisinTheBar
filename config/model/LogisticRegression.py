...
class LogisticRegression:
    ...
    def _forward_and_backprop(self, X_values_Train_Val_OrTest: np.ndarray, y_values_Train_Val_OrTest: np.ndarray) -> tuple:
        """ 
        predict, calculate loss, calculate gradients and REpeat.
        Returns the loss + how to update weights.
        """ 
        X = X_values_Train_Val_OrTest
        y = y_values_Train_Val_OrTest 
        # Forward: calculate preds
        ...
        # Quickk Runtime CHECK - Make sure loss is finite! Common issue I've had in training b4
        self._validate_loss_finite(nll)
        
        # Backprp: calculate grads
    ...
    def _confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
        """Compute confusion matrix for bin classification in a cool way.
        """
        ...
        # Uses the 0 and 1s as co-ords of the 2 by 2 matrix :)
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray, x_test: np.ndarray, y_test: np.ndarray) -> dict:
        ...
        for epoch in range(self.max_iterations):
            ...
            if epoch % 100 == 0:
                ...
                # Make preds just for accuracy purposes using current weights
    ...
    def print_confusion_matrix_stats(self) -> None:
        """Print confusion matrix statistics for test set only."""
        print("\n" + "="*60)
        print("Conf Matrix Statistics ")
        print("="*60)
         
        class_names = {0: "Besni", 1: "Kecimen"}
        cm = self.history['test_cm']
